...
internal static class RawSetNamePatch
{
    ...
    {
        if (!AmongUsClient.Instance.AmHost || !TOHFE.GameStates.InGame) return true;
        ...
        _ = new LateTask(() =>
        {
            switch (exception)
            {
                case true when __instance != null:
                    TOHFE.Logger.Warn($"Failed to set name for {__instance.GetRealName()}, trying alternative method", "RawSetNamePatch");
                    __instance.transform.FindChild("Names").FindChild("NameText_TMP").GetComponent<TextMeshPro>().text = name;
                    TOHFE.Logger.Msg($"Successfully set name for {__instance.GetRealName()}", "RawSetNamePatch");
                    break;
                case true:
