...
public static class CustomSoundsManager
{
...
#if ANDROID
    private static readonly string SOUNDS_PATH = Path.Combine(UnityEngine.Application.persistentDataPath, "TOHFE-DATA", "resources");
#else
    ...
    public static void Play(string sound)
    {
        ...
        if (!File.Exists(path))
        {
            var stream = Assembly.GetExecutingAssembly().GetManifestResourceStream("TOHFE.Resources.Sounds." + sound + ".wav");
            if (stream == null)
            {
